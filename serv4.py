...
class TemperatureOPCUAServer:
    ...
    async def create_sensor_node(self, building, room, pc, hardware_type, hardware_name, sensor_index, sensor_name):
        ...
        try:
            ...
            temp_var = await self.sensors_root.add_variable(
                ua.NodeId(node_id, self.namespace_idx),  # ИСПРАВЛЕНИЕ: указываем точный NodeID
                display_name,
                0.0,
    ...
    async def start(self):
        ...
        try:
            ...
            # Создаем несколько тестовых узлов для демонстрации
            await self.create_test_nodes()
    ...
    async def create_test_nodes(self):
        """Создание тестовых узлов для типичных датчиков"""
        print("INFO: Создание базовых узлов для типичных датчиков...")
        
        # Примеры типичных датчиков
        test_sensors = [
            (1, 101, 1, "CPU", "Intel Core i5-7400", 0, "CPU Core #1"),
            (1, 101, 1, "CPU", "Intel Core i5-7400", 1, "CPU Core #2"),
            (1, 101, 1, "CPU", "Intel Core i5-7400", 2, "CPU Core #3"), 
            (1, 101, 1, "CPU", "Intel Core i5-7400", 3, "CPU Core #4"),
            (1, 101, 1, "CPU", "Intel Core i5-7400", 4, "CPU Package"),
            (1, 101, 1, "SuperIO", "Nuvoton NCT6776F", 0, "CPU Core"),
            (1, 101, 1, "SuperIO", "Nuvoton NCT6776F", 1, "Temperature #1"),
            (1, 101, 1, "SuperIO", "Nuvoton NCT6776F", 2, "Temperature #2"),
            (1, 101, 1, "SuperIO", "Nuvoton NCT6776F", 3, "Temperature #3"),
            (1, 101, 1, "GpuNvidia", "NVIDIA GeForce GTX 1060 6GB", 0, "GPU Core"),
            (1, 101, 1, "HDD", "WDC WD10EZRZ-00HTKB0", 0, "Temperature"),
            (1, 101, 1, "HDD", "Apacer AS350 256GB", 0, "Temperature"),
        ]
        
        for building, room, pc, hw_type, hw_name, sensor_idx, sensor_name in test_sensors:
            await self.create_sensor_node(building, room, pc, hw_type, hw_name, sensor_idx, sensor_name)
        
        print(f"INFO: Создано {len(test_sensors)} базовых узлов датчиков")
    ...
    def print_status(self):
        ...
        if self.nodes:
            ...
            print(f"   • Подключенных ПК: {len(computers)}")
            for comp, sensors in computers.items():
                hw_types = set(s['hardware_type'] for s in sensors)
                print(f"     - {comp}: {len(sensors)} датчиков ({', '.join(hw_types)})")
...
async def main():
    ...
    try:
        ...
        # Ждем завершения
        await asyncio.gather(monitor_task) #may add status_task
